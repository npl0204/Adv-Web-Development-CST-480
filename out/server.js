...
app.get("/book", (req, res) => {
    ...
    let filteredBooks = books.filter((book) => book.pub_year >= year);
...
app.post("/book", async (req, res) => {
    ...
    let id = books.length > 0 ? `${Math.max(...books.map((book) => Number(book.id))) + 1}` : "1";
    let INSERT_SQL = await db.prepare("INSERT INTO books(id, author_id, title, pub_year, genre) VALUES (?, ?, ?, ?, ?)");
    book.id = id;
    books.push(book);
...
app.put("/book/:id", async (req, res) => {
    ...
    await db.run(`UPDATE books set author_id = ?, title = ?, pub_year = ?, genre = ? WHERE id = ?`, [book.author_id, book.title, book.pub_year, book.genre, id]);
...
app.post("/author", async (req, res) => {
    ...
    let id = authors.length > 0 ? `${Math.max(...authors.map((author) => Number(author.id))) + 1}` : "1";
    let INSERT_SQL = await db.prepare("INSERT INTO authors(id, name, bio) VALUES (?, ?, ?)");
    author.id = id;
    authors.push(author);
...
app.put("/author/:id", (req, res) => {
    ...
    db.run(`UPDATE authors set name = ?, bio = ? WHERE id = ?`, [author.name, author.bio, id]);
    return res.sendStatus(200);
});
...
app.delete("/author/:id", async (req, res) => {
    ...
    let book = books.find((book) => book.author_id === id);
    if (book) {
